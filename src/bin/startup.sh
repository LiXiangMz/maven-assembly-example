...
if [ $pid_count -ne 0 ]; then
  echo "$APPLICATION is running, can't start again!"; exit 1;
fi
...
if [ -n "$SERVER_JMX_PORT" ]; then
  JAVA_OPTS="-Duser.timezone=GMT+8 -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.port=$SERVER_JMX_PORT $JAVA_OPTS"
fi


JAVA_OPTS="-Xms${HEAP_INIT:-128m} $JAVA_OPTS"

JAVA_OPTS="-Xmx${HEAP_MAX:-128m} $JAVA_OPTS"

JAVA_OPTS="-Xmn${HEAP_YOUNG_MAX:-128m} $JAVA_OPTS"
JAVA_OPTS="-Xss${X_S_S:-512k} $JAVA_OPTS -XX:LargePageSizeInBytes=256M"
JAVA_OPTS="$JAVA_OPTS ${JAVA_GC:-}"
#指定JVM内存
JAVA_OPTS="-server -XX:-OmitStackTraceInFastThrow -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=${BASE_PATH} $JAVA_OPTS"
...
# 打印启动命令
STARTUP_LOG="${STARTUP_LOG}application startup command: nohup java ${JAVA_OPTS} -jar ${BASE_PATH}/boot/${APPLICATION_JAR} --spring.config.location=${CONFIG_DIR} > ${LOG_PATH} 2>&1 &\n"


#======================================================================
# 执行启动命令：后台启动项目,并将日志输出到项目根目录下的logs文件夹下
#======================================================================
nohup java ${JAVA_OPTS} -jar ${BASE_PATH}/boot/${APPLICATION_JAR} --spring.config.location=${CONFIG_DIR} > ${LOG_PATH} 2>&1 &
